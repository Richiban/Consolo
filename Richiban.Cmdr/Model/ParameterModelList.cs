...
{
    internal class ParameterModelList : IReadOnlyList<ParameterModel>
    {
        private readonly List<ParameterModel> _parameterModels;

        public ParameterModelList(IEnumerable<ParameterInfo> parameters)
        {
            _parameterModels = parameters.Select(p => new ParameterModel(p)).ToList();
        }
        ...
        public IEnumerator<ParameterModel> GetEnumerator() =>
            _parameterModels.GetEnumerator();
        ...
        public CommandLineArgumentList ExpandShortForms(CommandLineArgumentList arguments)
        {
            foreach (var argument in
                arguments.OfType<CommandLineArgument.BareNameOrFlag>())
            {
