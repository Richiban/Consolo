...
namespace Richiban.Cmdr
{
    internal class ParameterModel
    {
        ...
        public ParameterModel(ParameterInfo parameterInfo)
        {
            ...
            var isParams = parameterInfo.IsDefined(
            ...
            AllowNoValues =
                parameterInfo.IsOptional || parameterInfo.ParameterType.IsArray;
        ...
        private static IReadOnlyList<ParameterName> BuildNamesList(
            ParameterInfo parameterInfo)
        {
            var shortFormAttributes = parameterInfo.GetCustomAttributes(inherit: true)
                .OfType<ShortFormAttribute>();

            var allNames = new List<ParameterName>();

            allNames.AddRange(
                shortFormAttributes.SelectMany(a => a.ShortForms)
            ...
            var parameterNameAttribute = parameterInfo.GetCustomAttributes(inherit: true)
            ...
            {
                allNames.AddRange(
                    parameterNameAttribute.Names.Select(
                        n => new ParameterName.LongForm(n)));
