...
record DiagnosticModel(string Code, string Message, Location? Location, DiagnosticSeverity Severity)
{
    public static DiagnosticModel NoMethodsFound() =>
        new DiagnosticModel(
            Code: "Consolo0002",
            Message:
            $"No command methods found. Make sure you're using the {ConsoloAttributeDefinition.ShortName} attribute.",
            Location: null,
    ...
    public static DiagnosticModel DuplicateCommand(
        string commandName, string parentCommandName, Option<Location> location) =>
        new DiagnosticModel(
            Code: "Consolo0003",
    ...
    internal static DiagnosticModel ErrorProcessingMethod(string message, Location? Location) =>
        new DiagnosticModel(
            Code: "Consolo0004",
            Message: message,
            Location: Location,
    ...
    internal static DiagnosticModel MethodMustBeStatic(IMethodSymbol methodSymbol) =>
        new DiagnosticModel(
            Code: "Consolo0005",
            Message:
            $"Method {methodSymbol} must be static in order to use the {ConsoloAttributeDefinition.ShortName} attribute.",
            Location: methodSymbol.Locations.FirstOrDefault(),
            Severity: DiagnosticSeverity.Error);

    internal static DiagnosticModel AttributeProblem(
        string errTypeName, CandidateReason candidateReason, Location? Location) =>
        new DiagnosticModel(
            Code: "Consolo0006",
    ...
    internal static DiagnosticModel UnsupportedParameterType(ParameterModel param) =>
        new DiagnosticModel(
            Code: "Consolo0008",
            Message:
            $"Parameter '{param.Name}' has a type that is unsupported ({param.Type.Name}).",
    ...
    internal static DiagnosticModel
        DuplicateNameOrAlias(string name, string commandName, Option<Location> location) =>
        new DiagnosticModel(
            Code: "Consolo0013",
            Message:
            $"The parameter name or alias '{name}' is already in use in command '{commandName}'.",
            Location: location | null!,
            Severity: DiagnosticSeverity.Error);

    internal static DiagnosticModel MainMethodFound(Option<Location> location) =>
        new DiagnosticModel(
            Code: "Consolo0014",
            Message: $"In order to use Consolo your program must NOT define a Main method",
            Location: location | null!,
            Severity: DiagnosticSeverity.Error);

    public static DiagnosticModel
        MethodScanInfo(ResultWithDiagnostics<IReadOnlyCollection<MethodModel>> methodResults) =>
        new(
            Code: "Consolo0000",
